...
class Hand:
    ...
    def __init__(self):
        self.cards = np.zeros(32, dtype=bool)
    ...
    def get_legal_called_aces(self) -> list[Suit]:
        ...
        for suit in [Suit.CLUB, Suit.SPADE, Suit.HEART]:
            if self.suit_counts[suit] == 0:
                continue
            if not self.cards[card_constants.aces[suit]]:
    ...
    def get_legal_cards_to_bury(self, called_ace : Suit | None) -> NDArray[np.bool]:
        if called_ace is None:
            ...
            return np.nonzero(self.cards & np.logical_not(card_constants.card_suit_masks[called_ace]))[0]

    # NOTE: This doesn't check for legality, returns score buried 
    def bury(self, card_int : int) -> int:
        self.cards[card_int] = False
        self.suit_counts[card_constants.card_suits[card_int]] -= 1
        self.number_of_cards -= 1
    ...
    def get_legal_cards_to_play(self, led_suit : Suit | None, called_ace : Suit | None) -> NDArray[np.bool]:
        if self.number_of_cards == 1:
            ...
            if self.suit_counts[led_suit] > 0:
                return np.nonzero(self.cards & card_constants.card_suit_masks[led_suit])[0]
            else:
                return np.nonzero(self.cards)[0]
        else:
            ...
            if has_called_ace:
                ...
                # If the called ace suit was not led and we have the ace, we cannot play it unless we have no other option
                    ...
                    if self.suit_counts[led_suit] > 0:
                        ...
                        return np.nonzero(self.cards & np.logical_not(card_constants.ace_masks[called_ace]))[0]
            elif self.is_picker:
                if self.suit_counts[led_suit] > 0:
                    return np.nonzero(self.cards & card_constants.card_suit_masks[led_suit])[0]
                elif self.suit_counts[called_ace] > 1:
